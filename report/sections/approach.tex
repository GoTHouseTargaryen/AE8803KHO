...
\noindent sorted in ascending order (lower is better), where
$\phi(S)$ is the pipeline-depth heuristic:
...
\noindent and $r_v$ is the periods remaining in crew vehicle $v$'s
rotation, $\mathcal{D} \setminus \mathcal{M}_{\mathrm{built}}$ is the
set of modules delivered to site but not yet assembled, and $J(S)$ is
the current weighted cost (Eq.~\ref{eq:cost}).

The primary tier guarantees that states completing more modules are
never discarded in favor of cheaper but idle states. The secondary
tier $\phi(S)$ rewards pipeline depth: delivered cargo, modules in
transit, and a fresh crew rotation each contribute positively, so the
beam diversifies across scheduling options rather than collapsing
to a single trajectory. The tertiary tier resolves ties by objective
value, allowing the cost-weight slider to discriminate between
otherwise equivalent schedules. Default beam width: $K = 100$.
